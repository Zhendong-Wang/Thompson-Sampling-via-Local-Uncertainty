...
class Variational_abl(BayesianNN):
    ...
    def sample_single_n(self, psi, sigma):
      N, dim = tf.shape(psi)[0], tf.shape(psi)[1]
      eps = tf.random_normal(shape=(1, dim))
      eps = tf.tile(eps, [N, 1])
      z = psi + eps * sigma
      return z
    ...
    def decoder(self, x, z, output_dim, a_fn=tf.nn.relu):
	    ...
	    if self.glnoise:
		epsilon = tf.cast(Bernoulli(0.5).sample([tf.shape(x)[0], 15]), tf.float32)
	        zx = tf.concat([z, x, epsilon], axis=1)
	    else:
    ...
    def build_model(self):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope(self.name):
                ...
                self.q_mu, self.q_sigma = self.encoder(fake_x, self.latent_dim)

                if self.hparams.onez == 1:
                    self.z = self.sample_single_n(self.q_mu, self.q_sigma)
                else:
                ...
                fake_x = tf.ones(shape=tf.shape(self.x))
                self.y_pred_mu = self.decoder(self.x, self.z, self.num_actions)
