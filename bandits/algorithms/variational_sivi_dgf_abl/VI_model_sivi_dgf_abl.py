...
class VariationalSivi_dgf_abl(BayesianNN):
  ...
  def sample_single_n(self, psi, sigma):
      N, J, dim = tf.shape(psi)[0],  tf.shape(psi)[1],  tf.shape(psi)[2]
      eps = tf.random_normal(shape=(1, J, dim))
      eps = tf.tile(eps, [N, 1, 1])
      z = psi + eps * sigma
      return z
  ...
  def decoder(self, x, z, output_dim, a_fn = tf.nn.relu):
	  ...
	  if self.glnoise:
            epsilon = tf.cast(Bernoulli(0.5).sample([tf.shape(x)[0], self.J, half_latent_dim]), tf.float32)
  ...
  def build_model(self):
    ...
    with self.graph.as_default():
      ...
      with tf.name_scope(self.name):
        ...
        fake_x = tf.ones(shape=tf.shape(self.x))
        self.z_logv = self.sample_logv(fake_x, self.latent_dim)
	...
	#self.q_sigma = tf.tile(tf.reshape(q_sigma, [1, -1]), [tf.shape(self.x)[0], 1])
        ...
        self.q_mu_K = self.sample_psi(fake_x, self.noise_dim, self.implicit_hidden_layer_dim,
                                                      self.K, self.latent_dim, reuse=False)

        self.q_sigma_K = tf.tile(tf.expand_dims(self.q_sigma, axis=1), [1, self.K, 1])

        self.q_mu_J = self.sample_psi(fake_x, self.noise_dim, self.implicit_hidden_layer_dim,
                                                      self.J, self.latent_dim, reuse=True)

        self.q_sigma_J = tf.tile(tf.expand_dims(self.q_sigma, axis=1), [1, self.J, 1])

        if self.hparams.onez == 1:
            self.z_J = self.sample_single_n(self.q_mu_J, self.q_sigma_J)
        else:
	...
	self.prior_sigma = tf.Variable(self.psigma)
 	...
 	#self.log_prior_prob = - tf.reduce_sum(tf.square(self.z_J / self.prior_sigma), axis=2)	
        ...
        self.y_pred_mu = self.decoder(self.x, self.z_J, self.num_actions)
  ...
  def train(self, data, batch_size, num_steps, global_t, initial_lr):
    ...
    with self.graph.as_default():
      ...
      for step in range(num_steps):
        ...
        if self.hparams.km == 1:
            kl_weight = 1.0 / ((self.times_trained + 1) * 20)
        else:
            kl_weight = 1.0
