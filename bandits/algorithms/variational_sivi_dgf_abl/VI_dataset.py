...
class VIContextualDataset(object):
  ...
  def get_batch_with_weights(self, batch_size):
    ...
    n, _ = self.contexts.shape
    sample_size = batch_size #min(n, batch_size)
    if self.buffer_s == -1:
      # use all the data
      ind = np.random.choice(range(n), sample_size)
    else:
      # use only buffer (last buffer_s obs)
      ind = np.random.choice(range(max(0, n - self.buffer_s), n), sample_size)

    weights = np.zeros((sample_size, self.num_actions))
  ...
  def get_batch_with_weights_2(self, batch_size, replace=False):
    """Returns a random mini-batch with one-hot weights for actions."""
    n, _ = self.contexts.shape
    if batch_size > n and not replace:
      batch_size = n
    if self.buffer_s == -1:
      # use all the data
      if not replace:
        ind = np.random.permutation(n)[:batch_size]
      else:
        ind = np.random.choice(range(n), batch_size)
    else:
      # use only buffer (last buffer_s obs)
      if not replace:
        ind = np.random.shuffle(range(max(0, n - self.buffer_s), n))[:batch_size]
      else:
        ind = np.random.choice(range(max(0, n - self.buffer_s), n), batch_size)
